...
Run with: uvicorn app.api.server:app --host 127.0.0.1 --port 8000
"""
...
def require(user, permission: str) -> None:
    if not has_permission(user.role, permission): raise HTTPException(status_code=403, detail="Insufficient permissions")
...
def register(body: Credentials):
    try: user = auth.register(body.username, body.password)
...
def login(body: Credentials):
    try: access, refresh = auth.login(body.username, body.password)
...
def logout(authorization: str | None = Header(default=None)):
    current_user(authorization); auth.logout(authorization[7:].strip()); return {"status": "ok"}


@app.get("/auth/me")
def me(authorization: str | None = Header(default=None)):
    user = current_user(authorization); return {"id": user.id, "username": user.username, "role": user.role}


@app.get("/devices")
def list_devices(authorization: str | None = Header(default=None)):
    user = current_user(authorization); require(user, "captures:read"); return devices.list()


@app.post("/devices")
def create_device(body: DeviceCreate, authorization: str | None = Header(default=None)):
    user = current_user(authorization); require(user, "devices:control")
...
def list_measurements(limit: int = 100, authorization: str | None = Header(default=None)):
    user = current_user(authorization); require(user, "measurements:read"); return measurements.recent(limit)
