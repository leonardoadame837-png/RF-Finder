...
class RFService:
    def __init__(self, config=default_config, source=None, scan_interval_s: float = 0.5):
        self.config = config; self.source = source or self._build_source(config); self.analyzer = SpectrumAnalyzer(config); self.detector = SignalDetector(config); self.store = ObservationStore(config.database_path); self.telemetry = TelemetryStore(); self.agent = LocalSpectrumAnalyst(); self.scan_interval_s = max(0.05, float(scan_interval_s)); self._thread=None; self._stop=threading.Event(); self._lock=threading.RLock(); self._running=False; self._frame_index=0; self._last_error=None; self._latest=None; self._latest_detections=[]; self._waterfall=deque(maxlen=config.waterfall_history_frames); self._last_scan_at=None; self._lat=self._env_float("RF_FINDER_LAT"); self._lon=self._env_float("RF_FINDER_LON"); self._alt=self._env_float("RF_FINDER_ALT_M")
    ...
    def source_name(self):
        return source_provenance(self.source)["source"]
    ...
    def start(self):
        ...
        with self._lock:
            if self._running:self._thread=threading.Thread(target=self._run,name="rf-finder-scan",daemon=True); self._thread.start()
    ...
    def scan_once(self):
        result=self._process_frame(self.source.generate_frame(),self.source_type,datetime.now(timezone.utc).isoformat())
        for detection in self._latest_detections:
            simulated=detection.source_type is SourceType.SIMULATED
            self.store.add(classify_observation(RFObservation(timestamp=detection.timestamp,frequency_hz=detection.frequency_hz,peak_power_db=detection.power_dbfs,noise_floor_db=detection.noise_floor_dbfs,snr_db=detection.snr_db,bandwidth_hz=detection.bandwidth_hz,latitude=self._lat,longitude=self._lon,altitude_m=self._alt,source=detection.source_type.value,source_type=detection.source_type.value,signal_class="unknown",confidence=detection.confidence,evidence="simulated_signal" if simulated else "rf_measurement",simulated=simulated)))
        return result

    def import_measurement(self,iq,metadata=None):
        metadata=metadata or {}; source=normalize_source_type(metadata.get("source_type",SourceType.IMPORTED_MEASUREMENT.value)); timestamp=str(metadata.get("timestamp") or datetime.now(timezone.utc).isoformat()); return {"spectrum":self._process_frame(iq,source,timestamp) and self.latest_spectrum(),**self._process_frame if False else {}}

    def update_telemetry(self,payload): return self.telemetry.update(payload)
    def spectrum_agent_analysis(self): return self.agent.analyze(build_agent_context(self.latest_spectrum(),self.observations(limit=30)))

    def status(self):
        with self._lock:
            provenance=source_provenance(self.source); return {"running":self._running,"platform":platform.system().lower(),"client_architecture":"browser + local RF service","source":self.source_name,"source_type":provenance["source_type"],"source_status":self.source.status() if hasattr(self.source,"status") else {},"provenance":provenance,"frame_index":self._frame_index,"last_scan_at":self._last_scan_at,"last_error":self._last_error,"center_frequency_hz":self.config.center_frequency,"sample_rate_hz":self.config.sample_rate,"fft_size":self.config.fft_size,"gps":{"latitude":self._lat,"longitude":self._lon,"altitude_m":self._alt},"device_telemetry":self.telemetry.current(),"spectrum_agent":{"name":self.agent.name,"version":self.agent.version}}
    ...
    def detections(self,limit=250):
        with self._lock:return [d.to_dict() for d in self._latest_detections[:max(1,min(1000,limit))]]

    def observations(self,limit=250): return self.store.recent(limit)
