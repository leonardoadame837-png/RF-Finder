        ...
        def do_POST(self):
            ...
            try:
                ...
                if path == "/api/auth/logout":
                    self._require(None); api_auth.logout(self.headers.get("Authorization"), self.headers.get("Cookie")); self.send_header("Set-Cookie", f"{SESSION_COOKIE}=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0"); return self._send({"ok": True})
                if path == "/api/start":
