...
def demodulate(iq: np.ndarray, sample_rate_hz: float, mode: str, *, audio_rate_hz: int = 48_000, audio_bandwidth_hz: float | None = None) -> np.ndarray:
    ...
    if normalized == "am":
        return demodulate_am(iq, sample_rate_hz, audio_rate_hz=audio_rate_hz, audio_bandwidth_hz=10_000 if audio_bandwidth_hz is None else audio_bandwidth_hz)
    bandwidth = 15_000 if normalized in {"fm", "wfm"} else 5_000
    return demodulate_fm(iq, sample_rate_hz, audio_rate_hz=audio_rate_hz, audio_bandwidth_hz=bandwidth if audio_bandwidth_hz is None else audio_bandwidth_hz)
