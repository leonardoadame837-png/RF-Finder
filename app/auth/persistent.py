...
class PersistentAuthService:
    def __init__(self, db: Database):
        self.users = UserRepository(db); self.audit = AuditRepository(db)
        self.db = db

    def register(self, username: str, password: str, role: str = "viewer") -> User:
        username = username.strip().lower()
        if not username or len(password) < 12: raise ValueError("Username required and password must be at least 12 characters")
        if role not in {"viewer", "analyst", "operator", "admin", "owner"}: raise ValueError("Invalid role")
        if self.users.by_username(username): raise ValueError("User already exists")
        user = User(str(uuid.uuid4()), username, hash_password(password), role, True, datetime.utcnow())
        self.users.create(user); self.audit.record("user.register", user.id, "user", user.id)
        return user
    ...
    def login(self, username: str, password: str) -> tuple[str, str]:
        ...
        now = datetime.utcnow(); access = generate_access_token(); refresh = generate_refresh_token()
        ...
        self.db.execute("CREATE TABLE IF NOT EXISTS access_tokens (token_hash TEXT PRIMARY KEY, user_id TEXT NOT NULL, expires_at TEXT NOT NULL, FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE)")
    ...
    def authenticate(self, access_token: str) -> User | None:
        ...
        if datetime.utcnow() >= datetime.fromisoformat(row["expires_at"]):
            self.db.execute("DELETE FROM access_tokens WHERE token_hash=?", (hash_token(access_token),)); return None
    ...
    def logout(self, access_token: str) -> None:
        ...
        if user: self.audit.record("auth.logout", user.id, "session")
